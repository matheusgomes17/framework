    ...
    public function getSocialLinks()
    {
        ...
        if (config('services.bitbucket.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Bit Bucket']), 'bitbucket');
        }
        if (config('services.facebook.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Facebook']), 'facebook');
        }
        if (config('services.google.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Google']), 'google');
        }
        if (config('services.github.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Github']), 'github');
        }
        if (config('services.linkedin.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Linked In']), 'linkedin');
        }
        if (config('services.twitter.active')) {
            $socialite_enable[] = link_to_route('frontend.auth.social.login',
                __('labels.frontend.auth.login_with', ['social_media' => 'Twitter']), 'twitter');
        }
